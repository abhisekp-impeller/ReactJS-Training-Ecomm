...
export const checkAvailability = async <P extends WithAvailability & WithId>(
  product: P,
  { quantity, isThrow = true }: { quantity: number; isThrow?: boolean }
): Promise<number> => {
...
export const addToCart = async <
  P extends WithId & WithAvailability & WithPrice
>(
  product: P,
  { quantity = 1 }
) => {
  ...
  const newCart = produce(cart, (newCart) => {
    ...
    if (!cartItem) {
      ...
      const getExistingCartItemWithId = () =>
        newCart.items.find((item) => item.id === cartItem!.id);
    ...
    [newCart.price, newCart.quantity] = calculateCartItemsPriceQuantity(
      newCart.items
    );
...
export const calculateCartItemsPriceQuantity = <P extends WithPrice>(
...
export const createCartItem = <P extends WithPrice>(
  product: P,
...
export const deleteFromCart = async <P extends WithId & WithPrice>(
  product: P
) => {
  const cart = await getCart<P, Cart<P>>();
  const newCart = produce(cart, (newCart) => {
    ...
    [newCart.price, newCart.quantity] = calculateCartItemsPriceQuantity<P>(
      // @ts-ignore
      newCart.items
    );
