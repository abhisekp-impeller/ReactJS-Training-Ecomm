...
export const addProduct = async <
  P extends Pick<Product, "id" | "availability" | "price">
>(
  product: P
): Promise<P> => {
  if (!product.id) {
...
export const getProductById = async <P extends WithId>(
  id: string
): Promise<P | null> => {
  return await localforage.getItem(id);
};

export const updateProductById = async <P extends WithId>(
  id: string,
  productPartialUpdateData: any
): Promise<P> => {
  const products = await getProducts<P>();
  ...
  const product = Object.assign<P, Partial<P>>(
    products[index],
...
export const deleteProduct = async <P extends WithId>(
  query: any
): Promise<P[]> => {
  const products = await getProducts<P>();
  const [productsToDelete, productsToKeep] = partition<P>(sift<P>(query))(
    products
  );
...
export const softDeleteProductById = async <P extends WithId>(
  id: string
): Promise<P> => {
